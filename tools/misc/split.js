...
//
//    node diskimage.js --all="*.{ZIP,ARC}" --list=csv >> archive.csv
//
...
let csvFile = args[i++];
if (!csvFile) {
    console.log("Usage: node --max-old-space-size=10240 split.js <csvfile>");
    process.exit(1);
}
...
for (let line of lines) {
    ...
    if (match) {
        ...
        let remainder = match[7];
        match = remainder.match(/^"([^"]+)",(.*)$/);
        let fullPath = match? match[1] : (match = remainder.match(/^([^,]*),(.*)$/))[1];
        remainder = match[2];
        match = remainder.match(/^"([^"]+)"/);
        if (match) {
            let message = match[1];
        ...
        if (archiveName != archiveNext) {
            ...
            archiveLines.push(`${++archiveID},${archiveName},${remainder}`);
            archiveMode = true;
        }
        else if (fullPath == archiveName) {
            ...
            //
            fileLines.push(`${++fileID},${archiveID},${hash},${modified},${attr},${size},${compressed},${method},${fullPath},${remainder}`);
            archiveMode = false;
        }
    }
    else {
