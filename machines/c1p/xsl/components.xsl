...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="CREATOR">osi</xsl:variable>
	...
	<xsl:template name="componentScripts">
		...
		<script src="/machines/{$CREATOR}/{$APPCLASS}/releases/{$APPVERSION}/{$component}.js"> </script>
	</xsl:template>
		...
		<div id="{$id}" class="{$componentClass}" style="{$width}{$height}{$pos}{$left}{$top}{$padding}">
			...
			<xsl:if test="$component = 'machine'">
				...
				<div class="{$CSSCLASS}-copyright">
					<a href="{$SITEURL}" target="_blank">C1Pjs</a> v<xsl:value-of select="$APPVERSION"/> © 2012-2022 <a href="https://github.com/jeffpar" target="_blank">Jeff Parsons</a>
				</div>
