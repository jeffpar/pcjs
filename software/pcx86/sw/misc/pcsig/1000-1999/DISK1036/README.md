...
```
Disk No: 1036
Program Title: SURVEY SYSTEM version 2.0 (Disk 2 of 2)
PC-SIG version: 1.2

SURVEY SYSTEM is a useful program for creating attitude or climate
surveys.  It lets anyone with minimal skills on the IBM-PC or compatible
create their own survey.  It creates the kind of survey which makes a
general statement and the reader then chooses one of five levels of
agreement about that statement.  I like the idea of it being a complete
system.  You first create a pool of general survey statements/questions
in a master file.  The statements are then incorporated into your
specific survey.  They can be modified, deleted or moved around once on
the specific survey.  A neat feature of the program is it can place the
survey output into text files.  Once in these files, you can exit the
program and use any wordprocessor with them.  They print in a usable
format, but if you want to get fancy or use Ventura on them this could
be useful.  Overall the system is fairly competent at what it does.

Usage:  Survey Marketing/Database.

Special Requirements:  320K memory, a hard drive, and a printer is
optional.

How to Start:  Type GO (press enter).

Suggested Registration:  $35.00 for support and one year of updates.

File Descriptions:

CONFIG   FX   Configuration file.
GO       BAT  Batch file to display doc printing & loading instructions.
GO       TXT  Text of instructions to print & load the system.
INSTALL  BAT  Batch file to load files from diskette to hard drive.
S-CMNTS  DBF  Free form user comments data base.
S-DTAIL  DBF  Data file containing each question on a specific survey.
S-DTAIL  IDX  An index of the question detail file.
S-HELP   DBF  Data file containing helpful information.
S-HELP   IDX  Index file.
S-QUES   DBF  Data file containing each question in the master list.
S-QUES   IDX  Index of master questions in number order.
S-RSPNS  DBF  As the responses are received, they are stored here.
S-RSPNS  IDX  Index keeping all responses organized by survey.
S-SUMM   DBF  Temporary data file for compiling results during reports.
S-SURVEY DBF  The data file keeps track of each survey header.
S-SURVEY IDX  Index of all Surveys on the system.
S-SYST   DBF  Contains the printer control sequence & color/bw mode.
SSMAINA  BAT  Batch file to execute the SURVEY program.
SURVEY   DOC  Program Documentation.
SURVEY   FOX  The SURVEY program file.
...
```
 ...
 and install the system for immediate use:
   ...
   2. After booting your computer place the SURVEY diskette #2 in drive A:
   3. At the DOS prompt (C>, A>, etc.) enter: "TYPE A:SURVEY.DOC > LPT1:"
 ...
 To run Survey System on a dual-floppy system:
   1. Place disk #1 in drive A: and disk #2 in drive B:
   2. At the A> type "SSMAINA" and press ENTER.
...
```
          ...
          is strictly forbidden.  Copyright 1987,1988,1989.
          
          SURVEY SYSTEM (Ver 2.0)
