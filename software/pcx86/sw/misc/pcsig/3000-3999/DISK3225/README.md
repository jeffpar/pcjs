...
This package contains release 2.0 of the program INI-MANAGER.
All Rights are reserved.

This software is being distributed as shareware. The release you
have is free to use and to copy. 

If you agree with the software WIN-INIMAN you should become
...
-------------------
 ...
 - Statistic fnctions
 - Edit function with mini editor or notepad
 - Call Windows Setup/ Control Panel
...
Microsoft had a big succes with Windows version 3.0 and version 
3.1, too.

To handle Windows is much more easier than to work with a DOS shell. 
But - with the time the Windows user want to know more about the Windows 
internals. The most important Windows files are the both initialization 
files WIN.INI and SYSTEM.INI.

The Windows initialization files are very difficult to handle. The files 
are very complicated for normal user.

Every Windows user should have at least a bit know how about the Windows 
initialization files. 

The program INI-MANGER is an easy tool to handle the Windows INI-Files 
WIN.INI, SYSTEM.INI and other INI files. The user has an easy way to 
manage the INI files. On one side he has a compendium to all keynames and 
sections in the files, on the other side the program is an easy method to
update the files.

INI-MANAGER is the best way to learn more about your Windows 
configuration!
...
With the program INI-MANAGER you can manage all your Windows 
initialization files, especially the files WIN.INI and SYSTEM.INI. 

Both initialization files WIN.INI and SYSTEM.INI are very important 
for a Windows configuration. The Windows environment and the hardware 
requirements are described here. INI-MANAGER informs you about all the 
keynames in all the sections of the files. You can see standard values, 
possible values, version notes and all the possibilities for changing 
the key values in an easy way.

You can change a key value with the INI-MANAGER tool or with the Windows 
program Control Panel (if possible). For experts there is a mini editor 
implemented. With that function you can modify your entries directly.

When you start the program INI-MANAGER saves all important system files. 
When you have problems after some modifications you have an easy way to 
configurate your Windows system with the saved initialization files. 
Further the program is password protected. So no unauthorizied user can 
change your INI files with the INI-MANAGER.

All the dialog is in an interactive mode. The handle of the program is 
very easy. Ever Windows user can work with the program at once. With 
the help function you have access to all important information at once. 
...
INIMAN.EXE         Program <INI-MANAGER>
INIMAN.HLP         help file for Windows-Help
HELP.TXT           this file
...
Sample (copy from drive <A:> to drive/directory <C:\INIMAN>)
...
REGISTRATION / PURCHASE ORDER  Software utility INI-MANAGER
-----------------------------------------------------------

Please send me the software utility INI-MANAGER for a charge
of $ 49.--.

I take the folowing payment:
   (  ) cheque payment
	(only cheque payment from abroad)
