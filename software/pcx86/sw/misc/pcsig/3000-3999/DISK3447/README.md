...
{% raw %}
```
    This is an UNREGISTERED version of PC-Vault.  The administrator's name
    and/or password are displayed whenever a password is requested.  All
    features are fully functional.  A fully functional unregistered version
    of the optional HelpUser program is available from our BBS at (804)
    877-6261. An overview of HelpUser is provided in your PC-Vault
    administrator's manual.

    If after trying PC-Vault, you decide that you would like to purchase a
    registered version, please call us at (804) 872-9583.  If you wish to
    order by VISA or MasterCard, we can convert this unregistered version
    into the regular version over the phone.  We can also accept FAXed
    purchase orders from approved corporations.

    Johnson Computer Systems, Inc. is a member of the Association of
    Shareware Professionals (ASP).  For more information on ASP and their
    ombudsman program, please view OMBUDSMN.ASP.

                               Thank You
      
    Thank you for trying the PC-Vault Hard Disk Protection System.  All of
    the PC-Vault documentation is contained in the Administrator's Manual. 
    The User's Manual contains those portions of the Administrator's Manual
    which are pertinent to users who do not have administrator access.  A
    file named WHAT'S.NEW contains a list of features and other changes since
    version 4.4w for those of you who are familiar with that version.

    All three of these documents are contained in compressed form in the file
    VAULTDOC.EXE.  If your PC-Vault diskette is in drive A:, you may print
    either or both manuals by typing:

         A:VAULTDOC -p ADMIN.DOC       to print the Administrator's Manual,
         A:VAULTDOC -p USER.DOC        to print the User's Manual,
         A:VAULTDOC -p WHAT'S.NEW      to print the WHAT'S.NEW file, or
         A:VAULTDOC -p                 to print both manuals.

    If you wish to place a copy of any or all of these documents on your hard
    drive, simply make it the default drive and change to the directory you
    wish to contain the documents(s).  Then type one of the commands shown
    above omitting the -p.  For example, to place all documents in an
    existing C:\DOCS directory, use the commands:
    ...
    The manuals were compressed using the excellent PKZIP file compression
    ...
    Before calling for technical support, please see the In Case of
    Difficulty section in your administrator's manual.
