...
```
       ...
       _________ Register EXDEC 1.1--------------------: __$_5.00
  
       _________ Shipping/Handling for Enhanced Copy---: __$_2.50
                                          
                                                Total:    ________
  
  
  
       Disk Size___3.5___5.25************************************
...
```
       ...
       _________ Register ICONWAR 1.1------------------: __$10.00
...
```
           ...
           ICONWAR 1.1 (for Windows) is game where you and your
           computer opponent stake out territories on a 9 X 9 
           grid. Then taking turns, you each try to "hit" your
           opponent's locations. When you do hit one, you get
           another turn, until one of you has taken over all of
           the opponent's locations.

           When the game begins, you have the option of setting up
           from 5 (the default) to 25 territories each. You can also
           allow the enemy (the Badrovians) to auto-play (the default)
           or you may choose to initiate their turn each time. You can
           also disable the sound.

           Hardware requirements are the usual for any Windows programs,
           but as this game was written in Visual Basic, it also requires
           the file VBRUN100.DLL to be present in order to work. This
           file has been distributed with the program. You do need a mouse.

           Instructions are included within the program but it is not
           difficult to figure out and is suitable for almost any age.

           Yo will find, though that it may not be as easy as you might
           think to win. We have played many games in testing this program,
           and nobody ever shut-out the Badrovians. 

           You should have the following files:
                   ICONWAR.EXE      The main program
                   VBRUN100.DLL     Required to run
                   READIT.DOC       Additional documentation
                   READIT.COM        "          " self-displaying
                   INVOICE.DOC      Registration invoice
                   PRINV.BAT        To print invoice
                   WINSTAL.COM      Installation program
           

           INSTALLATION:

           We have included a simple installation program which will
           copy the needed files to any destination drive and directory
           specified. (WINSTAL.COM). To install, insert program disk
           (or a back-up) in disk drive. Then change to the disk drive
           by typing the drive letter (A, B, C or whatever it's 
           designation) then Type "WINSTAL (disk drivename) (destination
           drivename) (destination directory)" and then Enter. For
           example, to install from A: to C: in C:\WINDOWS directory
           you would type "WINSTAL A C WINDOWS". See DOS manual for
           more information on copying files.

           Once you have copied the files, you must go into your
           Windows program manager, select NEW, and set the program
           up there. The filename is ICONWAR.EXE.

           
           
           DISCLAIMER:

           We offer no warranty for ICONWAR Version 1.1 whether
           ...
           As this is a shareware program, you are permitted to copy
           and share it as desired. But, it must be copied in full,
           unaltered and no fee may be charged except a nominal copying
           fee by distributors. 

           If you like the program, you should register it with the author.
           The registration fee is $10.00, plus if you include an
           additional $2.50 for shipping/handling you will be sent the
           newest version which includes visual enhancements, many
           additional options and other improvements. You will also be
           sent a list of other programs.

           To register, if you have a printer, at the DOS prompt, type
          " PRINV ". There is a 'batch' program included which will
          print the file "INVOICE.DOC" which we would prefer you use
          to register. If you cannot use the invoice, then send the
          fee (U.S. Check or money order only) to:

              Eric Jarrett
              P.O. Box 34749
              Richmond, VA. 23234

            Please be sure to specify disk size!

         Write if you have any comments, complaints or suggestions. Or
         if you happen to shut-out the Badrovians.
...
```
       ...
       _________ Register QRUN 1.0---------------------: __$_5.00
