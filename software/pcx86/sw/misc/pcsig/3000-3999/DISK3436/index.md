...
MANUAL.DAT   DOS text file.  May be printed using the DOS "PRINT" command, but
             is printed with page numbers (12 pages in total) and proper
             margins etc. using PRINT!.EXE.  It contains instructions on
             running and using GUITAR TEACHER as well as valuable information
             on tuning and chord structure.
...
Crystal Displays.  If your screen seems to be displaying GUITAR TEACHER or the
printing utility improperly, run either program with a "/m" to force a
...
Filename: README
                         ...
                         GUITAR TUNER  Version 1.41


GUITAR TUNER is a six-string-guitar tuner.
...
README       This file.
TUNER.EXE    GUITAR TUNER.
PRINT!.EXE   Printing utility.  Prints User's Manual and Order Form.  
CONFIG.DAT   GUITAR TUNER data file.
CPS.DAT      GUITAR TUNER data file.
ERRORS.DAT   GUITAR TUNER data file.
MANUAL.DAT   DOS text file.  May be printed using the DOS "PRINT" command, but
             is printed with page numbers (6 pages in total) and proper
             margins etc. using PRINT!.EXE.  It contains instructions on
             running and using GUITAR TUNER as well as valuable information
             on tuning.

GUITAR TUNER will load and shutdown faster when run from a hard disk.  Simply
copy all the files with the DOS "COPY" command.  The manual provides further
instructions on copying files.

To run GUITAR TUNER or the printing utility, the default directory must be the
one where the GUITAR TUNER files are stored.

To run the printing utility type: print!

To run GUITAR TUNER type: tuner

GUITAR TUNER checks your computer's display adapter and automatically sets up
the program for either a monochrome or color display.  However, some adapters
cause GUITAR TUNER to display in color mode when in fact a monochrome monitor
is present.  This occurs most often with laptop Liquid Crystal Displays.  If
your screen seems to be displaying GUITAR TUNER or the printing utility
improperly, run either program with a "/m" to force a monochrome display.
