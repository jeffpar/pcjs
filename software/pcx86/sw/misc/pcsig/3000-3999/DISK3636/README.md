...
```
    ...
    MouseWarp Version 1.5
    (C) Copyright 1992 TOGGLE BOOLEANS
    All rights reserved.

    Features
    --------
    MouseWarp is a multi-purpose Windows mouse enchancement utility.
    With Mouse Warp, your meek mouse assumes greater abilities
    such as:

        - Changing your mouse pointer into various different shapes
          which make it more visible on laptops and high resolution
          monitors.

        - The selection of either right-handed or left-handed
          mouse pointers for the default pointer or any of the
          enhanced cursors provided by Mouse Warp.

        - Blinking after several seconds of mouse inactivity so when
          you come back to your computer you can find the mouse
          instantly.

        - The ability to move the mouse, in *any* application,
          by using the keyboard arrow keys while holding the
          Control and/or Alt keys.

        - The ability to wrap around the edges of the screen so
          if you go off the left side of the screen, you come
          back in on the right side.

    To select different mouse pointers or choose a left handed
    mouse pointer, select the Mouse Pointer entry from the
    MouseWarp system menu. You will be presented with views of the
    different cursors available.

    From the Preferences dialog box you can choose to enable
    blinking, keyboard mouse movement or mouse wrapping.

    With blinking enabled, the mouse will start to blink if you
    do not move the mouse for two seconds. This makes it easy
    to find the mouse if you get up from the computer and
    come back.

    When the keyboard mouse is enabled, you can move the mouse
    cursor solely from the keyboard. To move the mouse hold down
    the Control key and press the cursor arrow keys. This will
    move the mouse ten pixels in the direction you pressed. For
    fine movement, hold down the Alt key instead and for huge
    jumps across the screen, hold down both the Control and
    Alt keys at the same time. This can be handy on laptops or
    for precision graphics work.

    Mouse wrapping enables your mouse to wrap around the edges of
    the screen and come back in on the other side. This can be
    really handy when playing Solitare!

    Mouse Warp will remember your settings from one Windows session
    to the next so the next time you run it, you won't have to make
    the selections again.

    If you are running Windows 3.1, place MouseWarp in your Start
    Up folder and it will automatically load every time you start
    Windows with all your favorite settings enabled. If you use
    Mouse Warp just for the enhanced cursor shapes you can setup
    Mouse Warp to automatically exit after it sets the new mouse
    shape so it won't clutter up your desktop.
    To do so, select the MouseWarp icon and pick Properties from
    the Program Manager's File menu. At the end of the Command
    Line entry, add "/Q" (don't add the double quotes) and then
    click on OK. Note that all other MouseWarp features will be
    disabled if you do this. If at a later date you want Mouse Warp
    to stay on the desktop, just remove the "/Q".

    MouseWarp Version 1.5 is shareware. It may be copied and
    distributed freely under the following conditions:

        - No modifications are to be made to MouseWarp or this
          documentation.
        - This file must be distributed with the MouseWarp program.

    Suggestions
    -----------
    Any suggestions?

    When you register MouseWarp, send in any suggestions you have for
    other mouse features and we'll see if we can include them in the
    next release.


    Registering
    -----------
    To register your copy of Mouse Warp and receive the latest version,
    send a check or money order for $15 Canadian or $14 U.S to the
    address below.

    When you register your copy of Mouse Warp you will also receive
    a free copy of the TOGGLE BOOLEANS Screen Blanker!
    ...
    Just in case you're wondering, MouseWarp used to be called
    MousWrap in the old days when it only performed mouse wrapping.

    Disclaimer
    ----------
    The TOGGLE BOOLEANS MouseWarp is provided AS IS.
    ...
    the use or misuse of MouseWarp.
...
{% raw %}
```
WinOYE Version 1.1 (C) 1993 by Jose Rodriguez Alvira
...
<*> System requirements
	...
	VGA or EGA monitor (CGA not supported)
	Windows MIDI OUT driver
	Internal or external synthesizer
	Mouse
...
<*> Files included
	...
	WINOYE.EXE		Program file.
	WINOYE.HLP		Help
	WINOYE.XLW		Microsoft Excel workbook. Shows how to exchange WINOYE data with 
					programs that support DDE.
	README.TXT		This file.

<*> Installing WINOYE

	a) Be sure that your MIDI OUT driver is properly installed.
	b) Copy all files to the desired directory. 
	c) Run WINOYE. The program can optionally create a Program Manager group
	   and register WINOYE files extensions.	

<*> DDE Demo

	The Dymnamic Data Exchange demo is a Microsoft Excel file. It can only be run
	if you have Microsoft Excel installed on your system. For best results, run
	WinOYE first, then run the DDE demo.
	
<*> Uninstalling WINOYE

	a) Delete all the files in the WINOYE directory.
	b) Delete file WINOYE.INI in your WINDOWS directory.
	c) Delete WINOYE Program Manager group (if any).

	WINOYE does not modifies WINDOWS.INI, SYSTEM.INI, AUTOEXEC.BAT or CONFIG.SYS

<*> Registering WINOYE

	WINOYE is a shareware program. You have the right to use it for evaluation
	purposes only. If you like the program you should register your copy by
	sending a $20.00 check or money order drawn on a US bank to the following 
	address:
...
<*> Support
	...
	You can contact me by mail or by calling CompuServe (ID 70262,1562).
```
{% endraw %}
