---
layout: page
title: "PC-SIG Diskette Library (Disk #3798)"
permalink: /software/pcx86/sw/misc/pcsig/3000-3999/DISK3798/
machines:
  - id: ibm5170
    type: pcx86
...
{% raw %}
```
Using this program is pretty straightforward. You will probably 
not have any problem using it without reading the help file.


Apart from this one: if you want to change to a different drive
you press <Ctrl>+letter of the drive.



>>>> This is shareware, please register if you plan to use it <<<<


Registration
------------------------------------------------------------------
 
Registration is $20 and entitles you to two things: 
 
1     Use of this program as well as future updates. 
2     To have an influence on future updates. If you feel that there 
      are features missing that should be a part of this program, 
      please write me a description of 
 
		a) what the feature would do 
		b) how the user would command it 
 
     and I'll include it with the next version if I agree with you. 
 
To register, send $20 along with a note of your name and address to 
...
```
                           ...
                           RESETTING THE NAG PROGRAM

If you have set your Nag program to 'Check & Quit', you have reached
a Catch 22 situation.  The program runs as designed but it quits
before you have a chance to enter anymore data.

The quick way around this problem is to run the program with a
command-line parameter.  This temporary sets the program to
'Check & Run'.  It now runs as designed and the program remains
open, allowing you to enter data.  Run the program like this:

  NAG N

You can of course, set up a Program Item, in one of your folders, with
the command-line parameter entered.  Then all you have to do is click
the program icon.

You can also make Nag 'Check & Run' by editing the Nag initialization
file (NAG.INI). More information on doing this can be found in the
Nag help file (NAG.HLP).  The Nag help file can be viewed by using
...
Nag is a date reminder program for Windows 3.x
It is marketed as Shareware.   You should have received the
following files:
...
Feel free to contact us if you have suggestion for improvements.  We
can be reached at:
...
Nag has gone through several changes since it was first
released.  These changes have included the structure of
the Nag data files.

When Nag runs, it checks your Nag data files to see if they
need to be converted to the new file structure.  If they
need to be converted, routines are activated to do the
conversion.

Since nothing is completely fail-safe, problems may develop
during the conversion procedure and your Nag data files maybe
corrupted.  If they were, the two files affected would be:

  NAG.D8A   (Your event data)
  NAG.SAV   (Your old event data)

We highly suggest you print out the two files (if your old
version of Nag has the print feature) and then backup NAG.D8A and 
NAG.SAV, prior to running this newer version.
```
{% endraw %}
...
### Directory of PC-SIG Library Disk #3798

     Volume in drive A has no label
     Directory of A:\

    DIRCLIMB ZIP     47148   6-15-93   1:52p
    GRASP    ZIP     38449   6-15-93   1:52p
    NAG      ZIP     80776   6-15-93   1:53p
