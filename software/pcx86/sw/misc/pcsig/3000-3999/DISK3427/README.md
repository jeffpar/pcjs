...
{% raw %}
```
From dj@ssd.kodak.com Mon Oct 12 16:06:59 1992
Subject: About dropexit.zip

I thought you might be interested in bundling this with future
releases of Dropper.  Its a substitute for WinExit et al. that
tries to execute ExitWindows as soon as it's activated - the
idea is to put it on the Dropper desktop, preferably in a corner
where it's unlikely to be activated by mistake.  I set it up
so that by default it will ask for confirmation: this can be
overridden by launching it minimized.
...
dropper.wri is dead, long live dropper.hlp!

Documentation for Dropper may now be found in Windows Help format in the
file "dropper.hlp".

Changes since Dropper version 0.4
=================================

o	Fixed a couple of bugs in "Snap to Grid" and "Snap to Edge" which
    meant that they weren't consistent with "Arrange".
o	Fixed a but in Snap to Grid which resulted in icons "walking" across
    the screen when clicked upon.
o	Finally put a keyboard interface into the dialogs (why didn't I do
    this ages ago?).
o	Introduced an "About box".
o	"Arrange" now wraps round at the end of a column and won't overlay
    icons over others.
o	"Double Click" option.
o	"Hide when Minimized" option.
o	"Full Pathnames" option.
o	"Browse" button in the "Run program" dialog box.
o	Command line history in the "Run program" dialog box.
o	"Stay in Background" option reinstated by popular demand.
o	On-line help.
o	Work around for a Notepad "feature" which means that opening a file
    without an extension results in Notepad trying to open that file with
    ".TXT" appended.
o	More robust "HorribleHack"
o	Fixed a bug with the experimental version which resulted in screen
    savers not working and in Windows pausing every few seconds.
o	Misc. bug fixes (yes, yet more).
