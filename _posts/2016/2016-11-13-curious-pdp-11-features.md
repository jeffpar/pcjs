...
Last week, I ran PDPjs through a series of early [PDP-11 Paper Tape Diagnostics](/software/dec/pdp11/tapes/diag/).
These tests were originally released in 1970 and are documented in such DEC publications as the
[MAINDEC USER REFERENCE MANUAL](https://1drv.ms/b/s!ArcO_mFRe1Z9gp4RIs4XFo4GvOtA6Q) (October 1973).

[Tests 1-12](/software/dec/pdp11/tapes/diag/#tests-1-12) (MAINDEC-11-D0AA-PB through MAINDEC-11-D0LA-PB)
...
incorrect masks.  But aside from those hiccups, the tests ran fine.  Ditto for [Test 13](/software/dec/pdp11/tapes/diag/#test-13).

Things got much more interesting in [Test 14](/software/dec/pdp11/tapes/diag/#test-14).  For starters (and as DEC's documentation
...
After switching to a [PDP-11/20 configuration](/machines/dec/pdp11/1120/panel/debugger/), Test 14 uncovered a series of
problems, including:
...
Next up: [Test 15](/software/dec/pdp11/tapes/diag/#test-15), which uncovered a couple more problems:
...
successfully on an 11/70.  And [Test 15](/software/dec/pdp11/tapes/diag/#test-15) is not alone in that regard.
[Test 14](/software/dec/pdp11/tapes/diag/#test-14) also will not run properly on an 11/70; there again, the problem could have been
...
The most recent paper tape diagnostic I've run is the [11/70 CPU EXERCISER](/software/dec/pdp11/tapes/diag/#md-11-1170-cpu-exerciser),
...
It's worth noting that all these diagnostic paper tape images are in the [Absolute Loader](/software/dec/pdp11/tapes/absloader/) format,
but unlike tapes like [PDP-11 BASIC](/software/dec/pdp11/tapes/basic/), they don't include a start address, so you have to read the
documentation for each test to learn the starting procedure, including any switches that should be set first.

However, PDPjs makes life a bit simpler for you.  As noted for other [DEC PDP-11 Tape Images](/software/dec/pdp11/tapes/), any
"Absolute Format" tape can be read directly into RAM using the machine's **Read** button instead of **Load**, allowing you
to bypass the usual multi-step process of loading the [Bootstrap Loader](/software/dec/pdp11/boot/bootstrap/) in order to load the
[Absolute Loader](/software/dec/pdp11/tapes/absloader/) in order to load the desired tape.
